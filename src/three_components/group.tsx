...
import { SceneStoreType, useSceneStore } from '../vuer/sceneStore.tsx';
...
export function SceneGroup(
  ...
  const ops = useSceneStore(({ ops }) => ops);
  ...
  useEffect(() => {
    ...
    ops.rawUpdate({ position, rotation, scale });
...
export function GroupSlave({ children }: PropsWithChildren) {
  const { position, rotation, scale } = useSceneStore(({ position, rotation, scale }) => ({
    position,
    rotation,
    scale,
  })) as Sim3;
