...
export function PlyView({
  ...
  if (data && !data.attributes.normal) {
    if (data.index) {
      ...
      console.log(
        "Both data.index and data.attributes.normal are missing. Won't be able to compute surface normals.",
      );
    }
  }
  ...
  if (color) {
    return (
      <points ref={ref as MutableRefObject<Points>} {...rest}>
        <bufferGeometry {...data} />
        <pointsMaterial
          size={size}
          vertexColors
...
export function GltfView({
  ...
  useLayoutEffect(() => {
    if (matrix && matrix.length === 16) {
      ...
      data.scene.matrix.decompose(
        data.scene.position,
        data.scene.quaternion,
        data.scene.scale,
      );
